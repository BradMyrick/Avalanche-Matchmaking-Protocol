...
async fn main() -> Result<()> {
    ...
    // Default port 9317 (was previously 4317, the IANA-registered OTLP/gRPC
    // port — moved off 4317 because this service is NOT OTLP-compatible;
    // it speaks a custom Cap'n Proto schema, so any operator expecting
    // OTLP/Jaeger/Tempo ingestion would have been surprised).
    let addr_str = args.get(1).map(|s| s.as_str()).unwrap_or("127.0.0.1:9317");
        ...
        .run_until(async move {
            ...
            tokio::task::spawn_local(async move {
                ...
                if let Ok(mut w) = log_writer_clone.lock() {
                    let _ = w
                        .flush()
                        .map_err(|e| eprintln!("Failed to flush telemetry log: {}", e));
                }
                cancel_clone.cancel();
            });
