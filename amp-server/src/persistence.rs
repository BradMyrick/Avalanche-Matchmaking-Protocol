...
/// Bump this whenever a persisted struct's serialization changes in a way
/// `#[serde(default)]` cannot paper over. On open, a recorded version that
/// differs from this value is treated as incompatible (the alternative —
/// silently skipping entries that fail to deserialize — risks wiping the
/// entire DB on every boot after an upgrade, per the release audit §3b).
/// v1 = legacy (pre-versioned). v2 = current (adds ActiveMatch settlement
/// fields and relayer PendingSettlement fields, all serde-default-protected).
