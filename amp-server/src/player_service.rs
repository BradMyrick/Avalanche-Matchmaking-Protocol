...
impl PlayerServiceImpl {
    ...
    /// Symmetric two-player rating update for a settled 1v1 match. Snapshots
    /// BOTH players' `(mmr, rd, vol)` up front, computes both updates against
    /// the pre-match opponent values, and persists both.
    ///
    /// `score_a` is 1.0 (A wins), 0.5 (draw), or 0.0 (A loses). B's score is
    /// derived as `1.0 - score_a`.
    pub async fn record_match_result_symmetric(
        &self,
        player_a: &str,
        player_b: &str,
        game_id: &str,
        ...
        // Snapshot both profiles' rating tuples BEFORE either update. We
        // clone out of the DashMap guard scope to avoid holding shard locks
        // across the snapshot.
        let (a_pre, b_pre) = {
            let a = self
                .state
                .players
                .get(player_a)
                .ok_or_else(|| format!("Player {} not found", player_a))?;
            let b = self
                .state
                .players
                .get(player_b)
                .ok_or_else(|| format!("Player {} not found", player_b))?;
            (
                (a.global_mmr, a.mmr_uncertainty, a.mmr_volatility),
                (b.global_mmr, b.mmr_uncertainty, b.mmr_volatility),
            )
        };

        let score_b = 1.0 - score_a;

        // 2. Compute both updates against the pre-match snapshots. Pure math,
        //    no shared-state reads.
        let (a_new_mmr, a_new_rd, a_new_vol) =
            glicko2_update(a_pre.0, a_pre.1, a_pre.2, b_pre.0, b_pre.1, score_a);
        let (b_new_mmr, b_new_rd, b_new_vol) =
            glicko2_update(b_pre.0, b_pre.1, b_pre.2, a_pre.0, a_pre.1, score_b);

        info!(
            "Symmetric 1v1 rating update: {} {:.0} -> {:.0} (vs opponent pre={:.0}), {} {:.0} -> {:.0} (vs opponent pre={:.0})",
            player_a, a_pre.0, a_new_mmr, b_pre.0, player_b, b_pre.0, b_new_mmr, a_pre.0,
        );

        // Apply each update + persist.
    ...
    /// Apply an already-computed rating delta to a player profile and persist.
