...
- **Submitter Signature Verification**: `submitOutcome` rejects submissions whose EIP-712 signature does not recover to the caller's player_id (see `SECURITY_REVIEW.md` S1 and `docs/signing.mdx`).
...
- `AMP_ALLOW_UNAUTHENTICATED_RELAYER`: Set to `1` to bypass the API key requirement (**NOT RECOMMENDED** — see `SECURITY_REVIEW.md` S11).
...
## Security
See [`SECURITY_REVIEW.md`](../SECURITY_REVIEW.md) for the threat model and
