...
`privateKeyHex` (custodial) or a `signCallback` (wallet-integrated). See
[Security Review](/security-review) S2.
</Callout>
...
3. The server verifies the submitter signature recovers to the authenticated player (see [Security Review](/security-review) S1), then applies its own verifier EIP-712 signature.
...
- [ ] `RELAYER_API_KEY` set on both server and relayer (required by default — see [Security Review](/security-review) S11).
