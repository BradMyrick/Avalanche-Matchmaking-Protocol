...
| **JavaScript / TypeScript** | Node.js tooling | Alpha | `amp-sdk/js` |
...
for outcome signing. **No SDK silently generates an ephemeral identity** — see
the [Security Review](/security-review) S2.
</Info>
...
3. **Verify**: The server recovers the address from the signature and requires
   it to match the authenticated player's `player_id` (see
   [Security Review](/security-review) S1).
...
3. Transitive deps are pinned to avoid known-vulnerable versions (see
   [Security Review](/security-review) S12).

### Native C++

1. Use the `CMakeLists.txt` in `amp-sdk/examples/cpp` as a reference.
2. The C++ SDK does NOT bundle a Keccak-256 implementation — install
   `set_outcome_signer()` with a callback that uses a vetted library.
...
the caller (see [Security Review](/security-review) P8).
