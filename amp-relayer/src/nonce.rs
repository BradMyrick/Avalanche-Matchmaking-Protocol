...
impl NonceManager {
    ...
    ///
    /// The cached (hot) path atomically reads-and-increments under a single
    /// lock hold (see `reserve_cached`). The cold-start path awaits the
    /// provider, then re-checks the cache under the lock before inserting
    /// so a racing cold start cannot clobber a higher nonce.
    ...
    /// concurrent callers can never observe the same nonce.
...
mod tests {
    ...
    /// Concurrent reservations for the same address must never return a
    /// duplicate nonce.
    #[test]
