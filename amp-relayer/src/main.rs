...
// `#[sol(rpc)]` enables the provider-bound contract instance + call builders.
alloy_sol_types::sol! {
    #[sol(rpc)]
...
async fn run_settlement_processor(
    ...
    // Run N concurrent settlement workers. Claims are serialized by
    // `SettlementQueue::claim_lock`, but the slow chain submission (the
    // `await` in `process_next`) runs concurrently.
...
async fn main() -> Result<()> {
    ...
    // Prometheus metrics exporter (forward-looking — the counters are
    // recorded internally; the /metrics HTTP endpoint is a future milestone).
    let metrics_addr =
    ...
    // Refuse to start without an API key unless the operator explicitly opts
    // out. Without this, the relayer's submit_outcome endpoint is wide open
    ...
    if cfg.api_keys.is_empty() && !allow_unauth {
        ...
        tracing::warn!(
            "Relayer starting with no API keys. All incoming submit_outcome \
             requests will be accepted. This is insecure."
        );
    }
        ...
        .run_until(async move {
            tokio::select! {
                _ = tokio::signal::ctrl_c() => {
                    ...
                    // Flush before exit so terminal settlement state survives
                    // process exit.
                    if let Err(e) = queue.flush() {
                        tracing::error!("Failed to flush settlement queue on shutdown: {} (terminal state may be lost)", e);
                    } else {
                        info!("Settlement queue flushed.");
                    }
...
async fn accept_loop(
    ...
    loop {
        ...
        // Mutex poisoning is recovered so a poisoned per-IP lock does not
