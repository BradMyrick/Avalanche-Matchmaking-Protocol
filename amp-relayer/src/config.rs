...
pub struct Config {
    ...
    /// Number of concurrent settlement workers. Claims are serialized by
    /// `SettlementQueue::claim_lock`; the slow chain submission runs
    /// concurrently. Default 1 = behavior-preserving.
...
/// Constant-time comparison. Length is not treated as a secret here because
/// the inputs are SHA-256 hex digests (always 64 chars); for differing
/// lengths we still walk the shared prefix to avoid a length leak on
/// unequal-length inputs, then OR in any length difference.
...
/// Constant-time API-key verification. SHA-256 the candidate and compare
/// against EVERY stored hash with `ct_eq` without short-circuiting, so the
/// time does not depend on which (if any) key matched. The number of stored
