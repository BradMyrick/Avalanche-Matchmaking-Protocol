...
pub struct SettlementQueue {
    ...
    /// new work arrives instead of waiting on a fixed poll interval.
    notify: Arc<Notify>,
    /// Serializes the claim read-modify-write in `mark_in_flight` so multiple
    /// concurrent workers cannot double-claim the same task. Held only across
    /// the synchronous claim, never across an `await`, so slow chain
    /// submissions still parallelize across workers.
    claim_lock: Mutex<()>,
}
...
impl SettlementQueue {
    ...
    /// Synchronously flush all pending sled writes to disk. Call on shutdown
    /// so enqueued / in-flight / dead-letter records survive process exit.
    pub fn flush(&self) -> Result<(), RelayerError> {
        self.db.flush().map_err(RelayerError::Database)?;
        Ok(())
    }
    ...
    pub fn enqueue(&self, settlement: PendingSettlement) -> Result<(), RelayerError> {
        ...
        // rather than after the idle-poll fallback.
    ...
    pub fn mark_in_flight(&self) -> Result<Option<PendingSettlement>, RelayerError> {
        ...
        // both read the same Queued task and both mark it InFlight. The lock
        // is released before returning, well before any `await` in the caller.
    ...
    pub fn dead_letter(&self, mut settlement: PendingSettlement) -> Result<(), RelayerError> {
        ...
        // reconcile via getSettlementStatus.
    ...
    /// retries when many settlements fail simultaneously. Caps the exponent
    /// at 4 (16×) regardless of retry_count.
    ...
    async fn submit_settlement(
        ...
        // alloy's call builder sets EIP-1559 fields directly. Gas fields take
        // u128 on this alloy version.
...
/// `max_retries` is exceeded. Records a terminal `DeadLettered` result so
/// the server reconciliation loop can flag the desync.
fn handle_failure(
    db: &Db,
    max_retries: u32,
    match_id: &[u8],
