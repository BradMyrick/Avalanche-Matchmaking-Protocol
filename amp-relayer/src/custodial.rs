...
mod tests {
    ...
    /// Pinned known-answer vector for the HKDF-HMAC-keccak256 custodial
    /// derivation. keccak256 is not a standard HKDF hash, so no published
    /// reference vectors exist; this KAT pins the output for a fixed
    /// (master, purpose, gameId, chainId). Update deliberately if the scheme
    /// changes.
    #[test]
