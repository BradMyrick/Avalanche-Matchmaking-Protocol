...
`Portable.BouncyCastle 1.9.0` + `Newtonsoft.Json 13.0.3` to avoid
known-vulnerable transitive deps.
...
> **Note:** The C++ example needs a pre-computed auth signature. Set
> `AMP_EXAMPLE_SIGNATURE_HEX` (for local testing) or implement a real
> `signChallenge` callback in `examples/cpp/src/main.cpp`.
